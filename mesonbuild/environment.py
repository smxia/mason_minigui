...
from .mesonlib import (
    MesonException, EnvironmentException, MachineChoice, Popen_safe,Popen_safe_new,
...
class Environment:
    ...
    def _guess_nix_linker(self, compiler: T.List[str], comp_class: T.Type[Compiler],
        ...
        mlog.log("______________start to check__________________________")
        mlog.log("compiler ", compiler )
        mlog.log("for_machine", for_machine)
        mlog.log("comp_classs",comp_class)
        ...
        strvalue = ''+compiler[0]
        if( "arm-linux-gnueabi-gcc" in strvalue  or "arm-linux-gnueabi-g++" in strvalue ):
            extra_args += [' -L/home/dev/workspace/build-minigui-5.0/cross-build/arm64/rootfs/usr/local/lib']

        mlog.log("extra_args",extra_args)
        ...
        mlog.log("language binary value:", value)
        mlog.log("comp language ", comp_class.language)

        if( "arm-linux-gnueabi-gcc" in strvalue  or "arm-linux-gnueabi-g++" in strvalue ):
            #override = [' -L /home/dev/workspace/build-minigui-5.0/cross-build/arm64/rootfs/usr/local/lib']
            v='5.5.0'
            #check_args += override
        ...
        mlog.log("check_args ", check_args)


        if( "arm-linux-gnueabi-gcc" in strvalue  or "arm-linux-gnueabi-g++" in strvalue ):
            cmdstr = 'arm-linux-gnueabi-gcc  -Wl,--version   -I/home/dev/workspace/build-minigui-5.0/cross-build/arm64/rootfs/usr/local/include   -L /home/dev/workspace/build-minigui-5.0/cross-build/arm64/rootfs/usr/local/lib'
            mlog.log("command string value ", cmdstr)
            _, o, e = Popen_safe_new(cmdstr)
        else:
        ...
        if( "arm-linux-gnueabi-gcc" in strvalue  or "arm-linux-gnueabi-g++" in strvalue ):
            o ='GNU ld (Linaro_Binutils-2017.10) 2.25.2 Linaro 2016_02'

        v = search_version(o)
        if( strvalue == "arm-linux-gnueabi-gcc" or strvalue == "arm-linux-gnueabi-g++"):
            v='5.5.0'

        mlog.log("is gnu in o : ", 'GNU' in o)
        ...
        # first is for apple clang, second is for real gcc, the third is icc
            ...
            mlog.log("*******************************check failed")
            mlog.log('compiler ', compiler)
            mlog.log('for_machine', for_machine)
            mlog.log('linker_prefix',comp_class.LINKER_PREFIX)
            mlog.log('override', override)
            mlog.log('version',v)
            #cls = GnuBFDDynamicLinker
            #linker = cls(compiler, for_machine, comp_class.LINKER_PREFIX, override, version=v)
            raise EnvironmentException('Unable to determine dynamic linker')
    ...
    def _detect_c_or_cpp_compiler(self, lang: str, for_machine: MachineChoice) -> Compiler:
        ...
        mlog.log("********* is_cross", is_cross)
        mlog.log("******** info ", info)
        mlog.log("compilers", compilers)
        mlog.log("ccache", ccache)
        mlog.log("exe_wrap ", exe_wrap)
